...
colorArr = [(255,0,0), (255,127,0), (255,255,0), (0,255,0), (0,0,255), (75,0,130), (143,0,255)] #rainbow rgb values
...
try:
    ...
    #while True:
        ...
        if time.time() > (waveStart + 0.2): #shift wave when > then 0.2 seconds
            for i in range(32):
                ...
                colorindex = round((i+colorIterator)/7 % 1 * 7) #find color index for this bar
                xpos = 31-i
                draw.line((xpos, 0, xpos, volume), fill=colorArr[colorindex]) # draw amplitude line
                if volume > 3: #if high volume, add curve around bar
                    draw.line((xpos, 0, xpos, volume/2), width=5, fill=colorArr[colorindex])
                    draw.arc((xpos+1, 0, xpos+7, volume*2 - volume/3), 180, 270, fill=colorArr[colorindex])
                    draw.arc((xpos-7, 0, xpos-1, volume*2 - volume/5), 270, 0, fill=colorArr[colorindex])
                if colorIterator == 7: #iterator for shifting color array
                    colorIterator = 0
                else:
                    colorIterator += 1
